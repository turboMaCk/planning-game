...
join session tableId' name' status tables =
    let name =
            Text.strip name'
     in case Table.lookup tableId' tables of
            Just mvar -> do
...
handler state session id' conn = do
    ...
    case mTable of
        Just tableState -> do
            ...
            mConnId <-
                Concurrent.modifyMVar tableState $
                ...
                Just (player, connId) ->
                    ...
                    flip Exception.finally (disconnect tableState session connId) $ do
                        ...
                        WS.withPingThread conn 30 (pure ()) $ do
                            ...
                            if Player.numberOfConnections (Inc.unwrapValue player) == 1
                                then do
                                    table' <- Concurrent.readMVar tableState
        ...
        Just game' ->
            case Game.addVote session vote game' of
                Right newGames -> do
                    ...
                    if Game.allVoted (Table.players table) newGames
                        then do
                        ...
                        else pure $ table{Table.game = Just newGames}

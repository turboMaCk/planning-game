...
import PlanningGame.Data.Player (PlayerStatus (Active), Players)
...
getName name' games
  ...
  where
    ...
    checkName txt =
        if Set.member txt gameNames
            then checkName $ txt <> ".1"
            else txt
...
addVote sId vote (RunningGames past (RunningGame name votes isLocked)) =
    if isLocked
        then Left VotingEndedErr
        else Right $ RunningGames past $ RunningGame name updatedVotes False
  where
...
nextRound vote newName games@(RunningGames past current) =
    Right $
        RunningGames (finish vote current : past) $
...
allVotes games =
    reverse $
        (\FinishedGame{name, winningVote} -> (name, winningVote))
            <$> finishedGames games
...
playerVotes players games =
    reverse $
        (\game@FinishedGame{name} -> (name, playerVotes' game))
            <$> finishedGames games
  where
...
renameCurrent newName' original@(RunningGames finished' (RunningGame _ votes locked)) =
    if Text.null newName
        then original
        else RunningGames finished' $ RunningGame newName votes locked
  where
